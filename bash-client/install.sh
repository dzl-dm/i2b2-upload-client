#!/bin/bash
## Installer for DWH upload client on "git on windows"/git-bash
...
if ! pip install -r pseudonym/src/requirements.txt > /dev/null 2>&1 ; then
    ...
    python -m venv ./.venv
    ./.venv/bin/pip install -r pseudonym/src/requirements.txt
fi
...
scripts_path="$(dirname $(readlink -f ${BASH_SOURCE[0]}))/scripts"
bin_dir=$(find_bin_dir)
## Symlink into PATH
[[ -L ${bin_dir}/local-processing.sh ]] && rm ${bin_dir}/local-processing.sh
[[ -L ${bin_dir}/dwh-processing.sh ]] && rm ${bin_dir}/dwh-processing.sh
ln -s ${scripts_path}/local-processing.sh ${bin_dir}/local-processing.sh
ln -s ${scripts_path}/dwh-processing.sh ${bin_dir}/dwh-processing.sh

echo ""
echo "DWH Uploader Client installation complete"
echo -e "You can now use the scripts:\n - local-processing.sh\n - dwh-processing.sh"
echo ""
